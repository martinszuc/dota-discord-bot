...
class GameTimer:
    ...
    def __init__(self, guild_id, mode='regular'):
        ...
        logger.debug(f"Initialized GameTimer for guild ID {guild_id} in mode '{mode}'.")
    ...
    async def timer_task(self):
        ...
        self.time_elapsed += 1
        logger.debug(f"Time elapsed for guild ID {self.guild_id}: {self.time_elapsed} seconds")
        ...
        if self.time_elapsed >= 0:
            try:
                ...
                logger.error(f"Error in timer_task for guild ID {self.guild_id}: {e}", exc_info=True)
